<script setup>
import { checkCreateOrderAPI, checkGetAPI } from '@/apis/check'
...
const getCheckInfo = async () => {
  const { data } = await checkGetAPI()
  checkInfo.value = data.result
  addresses = data.result?.userAddresses
  curAddress.value = data.result?.userAddresses?.[0]
}
...
const confirm = () => {
  curAddress.value = addresses[addIndex.value]
}
const router = useRouter()
const createOrder = async () => {
  const { data } = await checkCreateOrderAPI({
    deliveryTimeType: 1,
    payType: 1,
    payChannel: 1,
    buyerMessage: '',
